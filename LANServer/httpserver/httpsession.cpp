...
#ifdef SUPERVERBOSE
        Logger::logger()->log(Logger::LANServer, "HttpSession: (constructor) new session %s with refCount=1",dataPtr->id.constData());
#endif
    }
    else
    {
...
HttpSession& HttpSession::operator= (const HttpSession& other)
{
    ...
    if (dataPtr)
    {
        ...
        dataPtr->refCount++;
#ifdef SUPERVERBOSE
        Logger::logger()->log(Logger::LANServer, "HttpSession: (operator=) session %s refCount=%i",dataPtr->id.constData(),dataPtr->refCount);
#endif
        dataPtr->lastAccess=QDateTime::currentMSecsSinceEpoch();
        dataPtr->lock.unlock();
    }
...
#ifdef SUPERVERBOSE
        Logger::logger()->log(Logger::LANServer, "HttpSession: (operator=) session %s refCount=%i",oldPtr->id.constData(),oldPtr->refCount);
#endif
        oldPtr->lock.unlock();
        if (refCount==0)
        {
...
#ifdef SUPERVERBOSE
        Logger::logger()->log(Logger::LANServer, "HttpSession: (destructor) session %s refCount=%i",dataPtr->id.constData(),dataPtr->refCount);
#endif
        dataPtr->lock.unlock();
        if (refCount==0)
        {
            Logger::logger()->log(Logger::LANServer, "HttpSession: deleting data");
            delete dataPtr;
        }
    }
}
