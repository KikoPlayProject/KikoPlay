    ...
    :QObject(parent)
{
    ...
    Logger::logger()->log(Logger::LANServer, "HttpSessionStore: Sessions expire after %i milliseconds",expirationTime);
}
...
QByteArray HttpSessionStore::getSessionId(HttpRequest& request, HttpResponse& response)
{
    ...
    if (!sessionId.isEmpty())
    {
        ...
        {
            Logger::logger()->log(Logger::LANServer, "HttpSessionStore: received invalid session cookie with ID %s",sessionId.data());
            sessionId.clear();
        }
    }
    mutex.unlock();
...
HttpSession HttpSessionStore::getSession(HttpRequest& request, HttpResponse& response, bool allowCreate)
{
    ...
    if (allowCreate)
    {
        ...
        Logger::logger()->log(Logger::LANServer, "HttpSessionStore: create new session with ID %s",session.getId().data());
...
void HttpSessionStore::sessionTimerEvent()
{
    ...
    while (i != sessions.end())
    {
        ...
        {
            Logger::logger()->log(Logger::LANServer, "HttpSessionStore: session %s expired",session.getId().data());
