...
{
    #ifdef SUPERVERBOSE
        Logger::logger()->log(Logger::LANServer, "HttpRequest: read request");
    #endif
    ...
    {
        #ifdef SUPERVERBOSE
            Logger::logger()->log(Logger::LANServer, "HttpRequest: collecting more parts until line break");
        #endif
        return;
    }
    ...
    {
#ifdef QT_DEBUG
        Logger::logger()->log(Logger::LANServer, "HttpRequest: from %s: %s",qPrintable(socket->peerAddress().toString()),newData.data());
#endif
        QList<QByteArray> list=newData.split(' ');
        if (list.count()!=3 || !list.at(2).contains("HTTP"))
        {
            Logger::logger()->log(Logger::LANServer, "HttpRequest: received broken HTTP request, invalid first line");
            status=abort;
        }
        else
        {
...
void HttpRequest::readHeader(QTcpSocket* socket)
{
    ...
    if (colon>0)
    {
        ...
        #ifdef SUPERVERBOSE
            Logger::logger()->log(Logger::LANServer, "HttpRequest: received header %s: %s",currentHeader.data(),value.data());
        #endif
    }
    else if (!newData.isEmpty())
    {
        ...
        #ifdef SUPERVERBOSE
            Logger::logger()->log(Logger::LANServer, "HttpRequest: read additional line of header");
        #endif
    ...
    else
    {
        ...
        #ifdef SUPERVERBOSE
            Logger::logger()->log(Logger::LANServer, "HttpRequest: headers completed");
        #endif
        ...
        {
            #ifdef SUPERVERBOSE
                Logger::logger()->log(Logger::LANServer, "HttpRequest: expect no body");
            #endif
            status=complete;
        }
        else if (boundary.isEmpty() && expectedBodySize+currentSize>maxSize)
        {
            Logger::logger()->log(Logger::LANServer, "HttpRequest: expected body is too large");
            status=abort;
        }
        else if (!boundary.isEmpty() && expectedBodySize>maxMultiPartSize)
        {
            Logger::logger()->log(Logger::LANServer, "HttpRequest: expected multipart body is too large");
            status=abort;
        }
        else {
            #ifdef SUPERVERBOSE
                Logger::logger()->log(Logger::LANServer, "HttpRequest: expect %i bytes body",expectedBodySize);
            #endif
            status=waitForBody;
        }
    }
...
void HttpRequest::readBody(QTcpSocket* socket)
{
    ...
    if (boundary.isEmpty())
    {
        ...
        #ifdef SUPERVERBOSE
            Logger::logger()->log(Logger::LANServer, "HttpRequest: receive body");
        #endif
    ...
    else
    {
        ...
        #ifdef SUPERVERBOSE
            Logger::logger()->log(Logger::LANServer, "HttpRequest: receiving multipart body");
        #endif
        ...
        {
            Logger::logger()->log(Logger::LANServer, "HttpRequest: received too many multipart bytes");
            status=abort;
        }
        else if (fileSize>=expectedBodySize)
        {
        #ifdef SUPERVERBOSE
            Logger::logger()->log(Logger::LANServer, "HttpRequest: received whole multipart body");
        #endif
            tempFile->flush();
            if (tempFile->error())
            {
                Logger::logger()->log(Logger::LANServer, "HttpRequest: Error writing temp file for multipart body");
            }
            parseMultiPartFile();
            tempFile->close();
            status=complete;
...
{
    #ifdef SUPERVERBOSE
        Logger::logger()->log(Logger::LANServer, "HttpRequest: extract and decode request parameters");
    #endif
    // Get URL parameters
...
{
    #ifdef SUPERVERBOSE
        Logger::logger()->log(Logger::LANServer, "HttpRequest: extract cookies");
    #endif
    foreach(QByteArray cookieStr, headers.values("cookie"))
    {
        ...
        {
            #ifdef SUPERVERBOSE
                Logger::logger()->log(Logger::LANServer, "HttpRequest: found cookie %s",part.data());
...
void HttpRequest::readFromSocket(QTcpSocket* socket)
{
    ...
    {
        Logger::logger()->log(Logger::LANServer, "HttpRequest: received too many bytes");
        status=abort;
    }
    if (status==complete)
    {
...
{
    Logger::logger()->log(Logger::LANServer, "HttpRequest: parsing multipart temp file");
    tempFile->seek(0);
    bool finished=false;
    while (!tempFile->atEnd() && !finished && !tempFile->error())
    {
        #ifdef SUPERVERBOSE
            Logger::logger()->log(Logger::LANServer, "HttpRequest: reading multpart headers");
        #endif
        ...
        while (!tempFile->atEnd() && !finished && !tempFile->error())
        {
            ...
            if (line.startsWith("Content-Disposition:"))
            {
                if (line.contains("form-data"))
                {
                    ...
                    #ifdef SUPERVERBOSE
                        Logger::logger()->log(Logger::LANServer, "HttpRequest: multipart field=%s, filename=%s",fieldName.data(),fileName.data());
                    #endif
                }
                else
                {
                    Logger::logger()->log(Logger::LANServer, "HttpRequest: ignoring unsupported content part %s",line.data());
                }
            }
        ...
        #ifdef SUPERVERBOSE
            Logger::logger()->log(Logger::LANServer, "HttpRequest: reading multpart data");
        #endif
        ...
        while (!tempFile->atEnd() && !finished && !tempFile->error())
        {
            ...
            if (line.startsWith("--"+boundary))
            {
                ...
                if (fileName.isEmpty() && !fieldName.isEmpty())
                {
                    ...
                    Logger::logger()->log(Logger::LANServer, "HttpRequest: set parameter %s=%s",fieldName.data(),fieldValue.data());
                }
                else if (!fileName.isEmpty() && !fieldName.isEmpty())
                {
                    ...
                    {
                        #ifdef SUPERVERBOSE
                            Logger::logger()->log(Logger::LANServer, "HttpRequest: finishing writing to uploaded file");
                        #endif
                        uploadedFile->resize(uploadedFile->size()-2);
                        uploadedFile->flush();
                        uploadedFile->seek(0);
                        parameters.insert(fieldName,fileName);
                        Logger::logger()->log(Logger::LANServer, "HttpRequest: set parameter %s=%s",fieldName.data(),fileName.data());
                        uploadedFiles.insert(fieldName,uploadedFile);
                        long int fileSize=(long int) uploadedFile->size();
                        Logger::logger()->log(Logger::LANServer, "HttpRequest: uploaded file size is %li",fileSize);
                    }
                    else
                    {
                        Logger::logger()->log(Logger::LANServer, "HttpRequest: format error, unexpected end of file data");
                    }
                }
            ...
            else
            {
                ...
                else if (!fileName.isEmpty() && !fieldName.isEmpty())
                {
                    ...
                    {
                        Logger::logger()->log(Logger::LANServer, "HttpRequest: error writing temp file, %s",qPrintable(uploadedFile->errorString()));
                    }
                }
            }
        }
    ...
    if (tempFile->error())
    {
        Logger::logger()->log(Logger::LANServer, "HttpRequest: cannot read temp file, %s",qPrintable(tempFile->errorString()));
    }
    #ifdef SUPERVERBOSE
        Logger::logger()->log(Logger::LANServer, "HttpRequest: finished parsing multipart temp file");
    #endif
}
